...
// Read a Zeeman Light Spectrum saved in binary format
// and returns an TH1D histogram (a pointer to)
TH1D* ReadZeemanSpectrum(const char *fname) {

  constexpr int npixels=7926;

  int16_t *image = new int16_t[npixels];

  try {
      std::ifstream rf(fname, ios::in | ios::binary);
      rf.read((char *) image, npixels*2);
      rf.close();
   ...
   if (gROOT->FindObject("hspectrum"))
       delete (TH1D *) gROOT->FindObject("hspectrum");

   TH1D *hspectrum = new TH1D("hspectrum","Source Spectrum",
                              npixels, 0, npixels);

   for (int i=0; i<npixels; i++) {
       hspectrum->SetBinContent(i, image[i]);
   }
   delete [] image;
