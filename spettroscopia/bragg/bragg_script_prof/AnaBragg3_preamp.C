...
//------------------------------------------------------------------------------------------------------------------------------
  ...
  int segnali_scartati_scarsa_qualita = 0;
...
/*    //CONTROLLI DI QUALITÀ SUL SEGNALE PREAMPLIFICATO
    
    // Flag to indicate if we should skip to the next iteration of the outer loop
    bool skip = false;
    
    // controllo sulle alteze del segnale
    for (int j = 0; j < 20; j++) {
        if (signal.s[j] > signal.s[(maxev - 1) - j]) {
        
          //std::cout << "basso:  " << signal.s[j] << std::endl;
          //std::cout << "alto:   " << signal.s[(maxev - 1) - j] << std::endl;
        
            skip = true;  // Set the flag to true
            break;  // Exit the inner loop
        }
    }

    // If the flag is set, continue to the next iteration of the outer loop = non considera questo segnale
    if (skip) {
        segnali_scartati_scarsa_qualita++ ;
        continue;
    }
*/
    //------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------
  ...
  std::cout << maxev - segnali_scartati_scarsa_qualita << " events analyzed..." << std::endl;
  std::cout << maxev << " total events " << std::endl;
